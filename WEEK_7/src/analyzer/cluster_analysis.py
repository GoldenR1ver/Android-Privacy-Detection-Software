...
1. 读取 sentences.jsonl（句向量）与可选 audit_processed.csv，合并 Lab3 行权三标签。
...
_DEFAULT_ITEMS = Path(__file__).resolve().parent / "ref" / "taxonomy" / "pipl_22_items.json"
_DEFAULT_LAW = Path(__file__).resolve().parent / "ref" / "taxonomy" / "pipl_law_reference_segments.json"
...
def merge_audit_into_sentences(
    ...
    for i, s in enumerate(sentences):
        ...
        if i < len(rows):
            a = rows[i]
            right = _parse_audit_int(a.get("right_claim"))
            method = _parse_audit_int(a.get("method_claim"))
            candidate = _parse_audit_int(a.get("app_test_candidate"))
            nr["right_claim"] = right
            nr["method_claim"] = method
            nr["app_test_candidate"] = candidate
            for extra in (
                "right_types",
                "execution_channels",
                "path_text",
...
def run(
    ...
    *,
    ...
    _plot_umap_colored_int(
        ...
        title="UMAP：Lab3 行权方式识别标签（优先级着色）",
        label_names={
            0: "无行权信号",
            1: "声明权利",
            2: "提供执行方式",
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="强化聚类：Lab3 行权三标签 + 22 项个保法维度 + UMAP/HDBSCAN + 法参考对比",
        ...
        epilog=(
            ...
            "若无 audit CSV，可省略 --audit-processed，则 Lab3 行权三标签全 0，仅做 22 项与聚类。"
        ),
    )
    p.add_argument(
        "--sentences-jsonl",
