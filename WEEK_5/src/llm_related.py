...
FEW_SHOT_HEADER = (
    "【参考例（人工核定，请与下列各例使用同一判标；仅最后一则「句子」需要你输出 JSON）】\n"
)


def build_user_prompt_with_shots(
    sentence: str,
    shots: Optional[List[Dict[str, Any]]],
) -> str:
    """在待判句前拼接 few-shot 块（与 USER_TEMPLATE 同一输出格式）。"""
    if not shots:
        return USER_TEMPLATE.format(sentence=sentence)
    blocks: List[str] = [FEW_SHOT_HEADER]
    for i, s in enumerate(shots, 1):
        text = str(s.get("text", "")).strip()
        rel = s.get("related")
        if not text or not isinstance(rel, bool):
            continue
        rh = str(s.get("reason_hint") or s.get("reason") or "").strip()
        if not rh:
            rh = "（口径与系统说明一致。）"
        example = json.dumps(
            {"related": rel, "confidence": 0.9, "reason": rh},
            ensure_ascii=False,
        )
        blocks.append(f"例{i}\n句子：{text}\n输出：{example}")
    blocks.append("")
    blocks.append(USER_TEMPLATE.format(sentence=sentence))
    return "\n".join(blocks)
...
def classify_ollama(
    ...
    user_content = build_user_prompt_with_shots(sentence, shots)
    payload = {
        "model": model,
        "stream": False,
        "messages": [
